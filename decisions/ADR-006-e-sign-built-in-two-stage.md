...
2. **The pre-check data validated the failure mode.** US-023's Phase 2 HIGH pre-checks (run 2026-05-16) found 2 `quote_signatures` rows referencing an `rfq_id` (`b3c02436-a08c-454b-9699-01ed3640b020`) that has no matching row in `public.rfqs`. Whether that came from the parent RFQ being deliberately deleted in dev or from a garbage/typo `rfq_id` at insert time is not knowable from the data alone — both branches are consistent with what was observed (`ip_address=::1`, `quote_total=1.00` placeholder, `quote_description='Testing'`). The cause-agnostic fact is the load-bearing one: **signature rows can end up referencing non-existent RFQs in this codebase**, and the absence of a foreign key is what allowed that invalid state in either branch. The new FK plus RESTRICT prevents both modes going forward — typo'd `rfq_id` values fail at insert time, and deliberate RFQ deletes are blocked until the dependent signatures are explicitly cleared.
