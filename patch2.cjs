...
console.log('Read', before, 'lines')

let patched = 0

// 1. Add showCreateDirectJob state
...
if (c.includes(oldState)) {
  // Only add if selectedJob not already there
  if (!c.includes('showCreateDirectJob')) {
    c = c.replace(oldState, newState)
    console.log('1. App state: PATCHED')
    patched++
  } else {
    console.log('1. App state: already present')
  }
} else { console.log('1. App state: NOT FOUND'); process.exit(1) }

// 2. Add New Job button next to New Work Order button
...
// 3. Wire up selectedJob state in JobBoard render
const oldJobBoard = `              : <JobBoard jobs={jobs} loading={jobsLoading} onCardClick={setSelectedJob} selectedId={selectedJob?.id} />}`
// This may already have setSelectedJob from our state addition - check if it's there
if (!c.includes('setSelectedJob') || !c.includes('selectedJob')) {
  const oldJobBoard2 = `              : <JobBoard jobs={jobs} loading={jobsLoading} onCardClick={`
  // find and replace just the onCardClick part if needed
  console.log('3. JobBoard render: checking...')
}
console.log('3. JobBoard render: OK (selectedJob state added in step 1)')

// 4. Add modal renders before closing of App return
const oldModals = `      {showCreateModal && <CreateRFQModal onClose={() => setShowCreateModal(false)} onCreated={handleRFQCreated} />}`
const newModals = `      {showCreateModal && <CreateRFQModal onClose={() => setShowCreateModal(false)} onCreated={handleRFQCreated} />}
...
if (c.includes(oldModals)) {
  c = c.replace(oldModals, newModals)
  console.log('4. Modal render: PATCHED')
  patched++
} else { console.log('4. Modal render: NOT FOUND'); process.exit(1) }

// 5. Insert CreateDirectJobModal component before // CREATE RFQ MODAL
const insertBefore = `// CREATE RFQ MODAL`

const directJobModal = `// CREATE DIRECT JOB MODAL
...
function CreateDirectJobModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const actionList = [
    { key: 'manufacture', label: 'Manufacture' }, { key: 'sandblast', label: 'Sandblast' },
    { key: 'prepare_material', label: 'Prepare Material' }, { key: 'service', label: 'Service' },
    { key: 'paint', label: 'Paint' }, { key: 'other', label: 'Other' },
    { key: 'repair', label: 'Repair' }, { key: 'installation', label: 'Installation' },
    { key: 'cut', label: 'Cut' }, { key: 'modify', label: 'Modify' },
  ] as const
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          {/* Client + Job No + Site Req */}
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-1">
              <label className="block text-xs font-medium text-gray-600 mb-1">Client *</label>
              <input value={clientName} onChange={e => setClientName(e.target.value)} placeholder="Client name..." className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div className="col-span-1">
              <label className="block text-xs font-medium text-gray-600 mb-1">Job Number</label>
              <div className="px-3 py-2 text-sm text-indigo-600 font-bold bg-indigo-50 rounded-lg border border-indigo-200">Auto (YY-XXXX)</div>
            </div>
            <div className="col-span-1">
              <label className="block text-xs font-medium text-gray-600 mb-1">Site Req / PO</label>
              <input value={siteReq} onChange={e => setSiteReq(e.target.value)} placeholder="e.g. PO-12345" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          {/* Work Type + Priority + Compiled By + Emergency */}
          <div className="grid grid-cols-4 gap-4 items-end">
            ...
            <div>
              ...
              <select value={priority} onChange={e => setPriority(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Compiled By</label>
              <input value={compiledBy} onChange={e => setCompiledBy(e.target.value)} placeholder="Name..." className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div className="flex items-center gap-2 pb-2">
              <input type="checkbox" id="emergency" checked={isEmergency} onChange={e => setIsEmergency(e.target.checked)} className="w-4 h-4 text-red-600" />
              <label htmlFor="emergency" className="text-sm font-medium text-red-600">Emergency</label>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={assignedEmployee} onChange={e => setAssignedEmployee(e.target.value)} placeholder="Employee name..." className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Supervisor</label>
              <input value={assignedSupervisor} onChange={e => setAssignedSupervisor(e.target.value)} placeholder="Supervisor name..." className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          {/* Actions Required */}
          <div>
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <table className="w-full text-xs">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-2 py-2 text-left text-gray-500 font-medium w-6">#</th>
                    <th className="px-2 py-2 text-left text-gray-500 font-medium">Description</th>
                    <th className="px-2 py-2 text-left text-gray-500 font-medium w-16">Qty</th>
                    <th className="px-2 py-2 text-left text-gray-500 font-medium w-20">UOM</th>
                    <th className="px-2 py-2 text-left text-gray-500 font-medium">Spec/Notes</th>
                    <th className="w-6"></th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-2">
              <input type="checkbox" id="hasDrawing" checked={hasDrawing} onChange={e => setHasDrawing(e.target.checked)} className="w-4 h-4" />
              <label htmlFor="hasDrawing" className="text-sm text-gray-700">Drawing Attached / Sketches</label>
            </div>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder="Additional notes..." className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" />
          </div>
        </div>
...
if (c.includes(insertBefore) && !c.includes('CreateDirectJobModal')) {
  c = c.replace(insertBefore, directJobModal + insertBefore)
  console.log('5. CreateDirectJobModal component: PATCHED')
  patched++
} else if (c.includes('CreateDirectJobModal')) {
  console.log('5. CreateDirectJobModal: already present')
} else {
  console.log('5. CreateDirectJobModal: INSERT POINT NOT FOUND')
  process.exit(1)
}
