...
- **2026-05-16** — US-024a (token closure) authored, applied, and verified. Two artefacts (committed as `fe67c2a`): (1) one-line code change in `api/sign-submit.js` adding `signature_token_id: tk.id` to the `quote_signatures` INSERT payload (verified by sole-writer audit on 2026-05-16 as the only `quote_signatures` write site in the codebase — Fast Track and all other paths bypass this table entirely; live DB-side trigger check returned zero writers), and (2) new migration `supabase/migrations/002_signature_token_id_not_null.sql` with a Phase-2-HIGH-pattern pre-check (`raise exception` + rollback if any quote_signatures row has NULL `signature_token_id`) followed by `ALTER COLUMN ... SET NOT NULL`. Discharges the deferred half of ADR-006's FK Activation & Delete-Semantics Amendment Decision 1. **Apply 2026-05-16:** Vercel build at `fe67c2a` confirmed Ready + Production; live IS NULL recheck returned 0; migration 002 applied via Supabase SQL Editor; post-apply verification confirmed `quote_signatures.signature_token_id` `is_nullable = NO`. Verified state tagged on `fe67c2a` as `phase1-us024a-token-closure-verified`.
