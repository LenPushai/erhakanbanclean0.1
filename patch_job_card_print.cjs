...
// Find handlePrintJobCard start and end
const startIdx = lines.findIndex(l => l.includes('const handlePrintJobCard = async'));
let endIdx = startIdx;
let depth = 0;
...
console.log('handlePrintJobCard lines:', startIdx+1, '-', endIdx+1);

const logoB64 = fs.existsSync('public/erha-logo.png')
  ? fs.readFileSync('public/erha-logo.png').toString('base64')
  : '';
...
const newFn = `  const handlePrintJobCard = async (job: Job) => {
    ...
    const logoHtml = '${logoSrc}'
      ? \`<img src="${logoSrc}" alt="ERHA" style="height:50px">\`
...
lines.splice(startIdx, endIdx - startIdx + 1, ...newFn.split('\n'));
fs.writeFileSync(filePath, lines.join('\n'), 'utf8');
console.log('PASS: Job card print template updated to match official ERHA layout');
