...
- **2026-05-16** — Phase 2 HIGH migration `supabase/migrations/001_signature_tables_phase2_high.sql` authored, applied, and verified via Supabase SQL Editor. Single combined file replaces the two-file plan originally drafted under Phase 2 HIGH; all three FKs use `ON DELETE RESTRICT`. All 6 post-apply verification checks passed: 3 FK constraints landed with `ON DELETE RESTRICT` (`signature_tokens_rfq_fk`, `quote_signatures_rfq_fk`, `quote_signatures_token_fk`); `quote_signatures.rfq_id` is `NOT NULL` (Finding G, landed early); `quote_signatures.signature_token_id` exists as nullable (NOT NULL deferred to US-024); 0 orphan rows remaining. Verified state tagged on commit `569f193` as `phase1-us023-schema-phase2-high-verified`.
- **2026-05-16** — US-024a (token closure) authored. Two artefacts: (1) one-line code change in `api/sign-submit.js` adding `signature_token_id: tk.id` to the `quote_signatures` INSERT payload (verified by sole-writer audit on 2026-05-16 as the only `quote_signatures` write site in the codebase — Fast Track and all other paths bypass this table entirely; live DB-side trigger check returned zero writers), and (2) new migration `supabase/migrations/002_signature_token_id_not_null.sql` with a Phase-2-HIGH-pattern pre-check (`raise exception` + rollback if any quote_signatures row has NULL `signature_token_id`) followed by `ALTER COLUMN ... SET NOT NULL`. Discharges the deferred half of ADR-006's FK Activation & Delete-Semantics Amendment Decision 1. **Strict 7-step application runbook (codified in the migration file's header):** (1) commit code + migration; (2) Vercel deploy + confirm live; (3) verify new signatures populate `signature_token_id` on the live deploy; (4) IS NULL count must be 0 against live quote_signatures (handle any gap rows from parallel-run signatures inserted between Phase 2 HIGH apply and the US-024a deploy); (5) THEN apply via Supabase SQL Editor; (6) post-apply verify; (7) tag `phase1-us024a-token-closure-verified`. The pre-check inside the migration is defence-in-depth, not a substitute for step 4.
