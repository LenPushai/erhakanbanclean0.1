...
export function SignaturePage() {
  ...
  // Stage 2 (US-013) checkpoint stub.
  if (tokenRecord.signature_stage !== 'manager') {
    return (
      <Centered>
        <div className="max-w-md bg-white rounded-xl shadow p-6 text-center">
          <h1 className="text-lg font-bold text-gray-900 mb-2">Customer signature page</h1>
          <p className="text-sm text-gray-600">Stage 2 (US-013) is implementation-pending. The token is valid and recognised — the customer signing interface is the next build.</p>
          <p className="text-xs text-gray-400 mt-4">Token: {token.slice(0,8)}...</p>
        </div>
      </Centered>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div className="bg-gradient-to-r from-blue-700 to-blue-900 text-white px-6 py-5">
          <h1 className="text-xl font-bold">Internal Quote Sign-off</h1>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-2 text-xs">
                <span className="text-gray-500">Draw your signature, or use the I Approve button below.</span>
            ...
            <button disabled={submitting} onClick={handleSubmit}
              ...
              {submitting ? 'Submitting...' : (signatureType === 'drawn' && signatureData ? 'Submit Signed Approval' : 'I Approve')}
            </button>
          </div>
        </div>
      </div>
...
function SuccessState({ stage }: { stage?: string }) {
  return (<Centered>
    <div className="max-w-md bg-white rounded-xl shadow p-6 text-center">
      ...
      <h1 className="text-lg font-bold text-gray-900 mb-1">{stage === 'manager' ? 'Quote signed and sent to customer' : 'Signature recorded'}</h1>
      <p className="text-sm text-gray-600">{stage === 'manager' ? 'A signing link has been emailed to the customer.' : 'Thank you.'}</p>
    </div>
  </Centered>)
}
