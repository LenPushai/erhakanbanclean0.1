...
// Change 1: Insert state after line 180 (after index 179)
console.log('Applying Change 1: state...');
lines.splice(180 + offset, 0, '  const [showCreateDirectJob, setShowCreateDirectJob] = useState(false)');
offset += 1;

// Change 2: Insert button after line 258 (after index 257)
console.log('Applying Change 2: New Job button...');
lines.splice(258 + offset, 0,
  "            {activeBoard === 'job' && (",
...
// Change 3: Insert modal render after line 275 adjusted
console.log('Applying Change 3: modal render...');
lines.splice(275 + offset, 0, '      {showCreateDirectJob && <CreateDirectJobModal onClose={() => setShowCreateDirectJob(false)} onCreated={fetchJobs} />}');
offset += 1;

// Change 4: Insert CreateDirectJobModal component before line 419 adjusted
console.log('Applying Change 4: component...');
...
'  const actionList = [',
"    { key: 'manufacture' as const, label: 'Manufacture' },",
"    { key: 'sandblast' as const, label: 'Sandblast' },",
"    { key: 'prepare_material' as const, label: 'Prepare Material' },",
"    { key: 'service' as const, label: 'Service' },",
"    { key: 'paint' as const, label: 'Paint' },",
"    { key: 'other' as const, label: 'Other' },",
"    { key: 'repair' as const, label: 'Repair' },",
"    { key: 'installation' as const, label: 'Installation' },",
"    { key: 'cut' as const, label: 'Cut' },",
"    { key: 'modify' as const, label: 'Modify' },",
'  ]',
'',
'  return (',
...
'          </div>',
'          <div className="grid grid-cols-2 gap-4">',
'            <div className="flex items-center gap-2"><input type="checkbox" id="djDrawing" checked={hasDrawing} onChange={e => setHasDrawing(e.target.checked)} className="w-4 h-4" /><label htmlFor="djDrawing" className="text-sm text-gray-700">Drawing Attached / Sketches</label></div>',
...
lines.splice(419 + offset - 1, 0, ...componentLines);
offset += componentLines.length;
...
const checks = [
  { name: 'State variable', pattern: 'showCreateDirectJob' },
  { name: 'New Job button', pattern: '<Plus size={15} />New Job' },
  { name: 'Modal render', pattern: 'showCreateDirectJob && <CreateDirectJobModal' },
  { name: 'Component definition', pattern: 'function CreateDirectJobModal' }
];
checks.forEach(check => {
  console.log(check.name + ': ' + (content.includes(check.pattern) ? 'PASS' : 'FAIL'));
});
